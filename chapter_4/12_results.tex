...
\begin{SCfigure}
  ...
  \caption{
    ...
    Panel (c) shows a simple peptide--antibiotic interaction model illustrating qualitatively distinct dynamics under: no drugs, antibiotic (slow constant decline), peptide (rapid initial decline), and peptide plus antibiotic (rapid initial decline followed by a constant slow decline).\vspace{1.8cm}
  }
...
\begin{figure*}
  \centering
  \begin{overpic}[width=\linewidth]{figures/distributions_triptych.pdf}
...
\begin{SCfigure}
\centering
\begin{overpic}[width = 10cm]{figures/surface_plot.pdf}
\put(10,97){\textbf{(a)}}
\put(10,50){\textbf{(c)}}
\put(50,97){\textbf{(b)}}
\put(50,50){\textbf{(d)}}
...
Panels (c,d) show the Bliss- and Loewe-based predictions for \(\psi\) over the mixing angle \(\phi\) along the observed isobole at \(\psi=\SI{0}{\per\hour}\) for (c) COL+PEN and (d) CHL+TET. \vspace{3.8cm}
}
