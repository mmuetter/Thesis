...
This is because an asymmetrical antagonism exists between the bactericidal antibiotic ceftazidime (drug A) and the bacteriostatic antibiotic tetracycline (drug B), resulting in a lower clearance rate for the combination A+B compared to drug A alone (\autoref{ssec:si_results}).
...
In our experiments, we found no evidence of a cost of resistance (see \autoref{ssec:si_methods}, \autoref{fig:plasmid_costs}, and \autoref{tab:man_ferroni}) or segregational loss (see \autoref{ssec:si_methods} and \autoref{tab:loss_data}).
...
Fourth, we observed that the number of single-resistant bacteria inoculating superinfections impacts the frequency of emerging double resistance (see \autoref{ssec:si_results}, \autoref{tab:encounter_advanced}).
