...
We replace each assay plate daily to renew the treatment and medium (\autoref{tab:encounter_advanced}).
...
\begin{SCfigure}
  ...
  \includegraphics[width=0.77\linewidth]{end_anova.pdf}
  \caption{
...
The \textit{prevention}~scenario is characterised by a moderate proportion of single-resistant admissions to the hospital ward, the absence of pre-existing double resistance, and a moderately spreading infection dynamic ($R_0 = 1.5$, \autoref{eq:R0},  \autoref{ssec:si_methods}).
In this scenario, there are no differences between combination, mixing, and cycling on the frequency of uninfected, single-resistant-infected and double-resistant-infected wells (\panelref{fig:end_anova}{A}, time series in \autoref{fig:exp1}).
...
All strategies (but not the control) were able to clear sensitive infections effectively with clearance probabilities of 97\% for drug A, 73\% for drug B, and 86\% for AB (\autoref{tab:clearance}).

\begin{figure}
  \centering
  \includegraphics[width=.9\linewidth]{scen3_paper.pdf}
  \captionsetup{skip=2pt}
  \caption{
...
In addition, we set the probability of admission and discharge to $\tau = 0.5$ and the infection probability to $\beta = 0.25$, resulting in a basic reproduction number $R_0 = 0.5$ (\autoref{eq:R0}).
...
Reducing the cell density for one or both single-resistant strains within a superinfected well reduces the probability of bacteria with complementary resistance to encounter and conjugate (see \autoref{ssec:si_results}).
...
Secondly, the treatment strategies influence the number of transferred single-resistant bacteria that inoculate superinfections by curbing the bacterial density within the infecting wells (see \autoref{ssec:si_results}).
...
\begin{figure*}[t]
  \newcommand{\panelwidth}{8cm}
  \newcommand{\capwidth}{6.6cm}

  \centering
  \begin{minipage}[t]{\panelwidth}
    \centering
    \includegraphics[width=\panelwidth]{20220412_f_emergence_anova.pdf}\\[2pt]
    \includegraphics[width=\panelwidth]{20220412_encounter_frequencies.pdf}\\[2pt]
    \includegraphics[width=\panelwidth]{20220412_conjugation_prop.pdf}
  \end{minipage}\hfill
  \begin{minipage}[t]{\capwidth}
    ...
    \captionof{figure}{
      ...
      Bars not sharing a letter are significantly different ($p < 0.05$, ANOVA tables and pairwise Tukey post hoc results can be found in \autoref{tab:emergence_anova} -- \autoref{tab:treatment_emergence_tukey}).
    ...
    Here the stars indicate predictions based on the ratio of superinfections per patient and the emergence per superinfection (\autoref{ssec: model}).
...
To address this, we developed a discrete-time stochastic model comprising 94 individual \textit{in silico} patients mimicking the epidemiological dynamics of the experiment (\autoref{ssec: model}).
The model was parametrised, but not fitted, with transition probabilities (\autoref{tab:m_none}--\autoref{tab:m1_ab}) that we estimated based on the transition frequencies measured \textit{in vitro}.
...
The aim of the \textit{validation simulations} is to recreate the experiments \textit{in silico} (\autoref{fig:stochs}B).
We found that the simulation results are in good agreement with the experimental data, indicating that the model reflects the dynamics observed in the \textit{in vitro} experiments well (see \autoref{fig:exp3}, \autoref{fig:exp1}, and \autoref{fig:exp2}).
...
We also observe some discrepancies for the spread of double resistance during the \textit{prevention}~scenario, which we attribute to contamination artefacts in the transition probabilities (see \autoref{ssec: model}).
...
In these \textit{variation simulations}, each of the 100 instruction sets was randomized based on the same three parameter sets used \textit{in vitro} (\autoref{fig:stochs}C).
...
We observed that the \textit{validation simulations} fluctuate around the \textit{variation simulations} and never diverge far (see \autoref{fig:exp3}, \autoref{fig:exp1}, and \autoref{fig:exp2}), indicating robustness of the experimental results to the randomisation of the instruction sets.

\paragraph{\textit{In silico} sensitivity analysis indicates that the superiority of combination therapy is robust.}
Given that the \textit{validation simulations} agreed well with the experiments, we used the model to perform an \textit{in silico} parameter sensitivity analysis of the experimental results (\autoref{fig:stochs}D).
...
It is the single best strategy in 55\% of the tested parameter sets with pre-existing double resistance and in 93\% of cases without pre-existing double resistance (\autoref{fig:senstivity_analysis}, \autoref{tab:win_loss_statistic_preex},~\autoref{tab:win_loss_statistic_no_preex}).
...
As expected, single-drug strategies perform particularly poorly when there is a high frequency of pre-existing single-resistance to the applied drug (\autoref{tab:worst_stats_no_preex}, \autoref{tab:worst_stats_preexisting}).
...
\begin{minipage}{12cm}
\centering
\includegraphics[width=\linewidth]{wins_and_losses_preexisting.pdf}\\[2pt]
\includegraphics[width=\linewidth]{wins_and_losses_no_preex.pdf}
\captionsetup{skip=0pt,width=\linewidth}
\caption{
...
\textbf{(A)} 10,000 parameter sets with pre-existing double resistance.
606/10,000 sets yielded no significant difference between the strategies.
\textbf{(B)} 10,000 parameter sets without pre-existing double resistance.
100/10,000 sets yielded no significant difference between the strategies.
}
