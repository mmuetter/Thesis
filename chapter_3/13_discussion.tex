...
When inferring growth rates from CFU, the observed rate of change $\psi_{\mathrm{CFU}}$ reflects both the rate of change of population size $\psi_B$ and changes in the probability that a plated bacterium forms a colony, $\eta$ (\autoref{eq:form_prob}).
...
This is because \(\psi_I\) reflects both the rate of population size change, \(\psi_B\), and changes in cell-specific luminosity (\autoref{eq:psi_I}); when population declines rapidly, \(\psi_I\) converges to \(\psi_B\).
