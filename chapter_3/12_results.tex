...
We found that light intensity increased linearly with bacterial density for signals above approximately \(20\;\mathrm{rlu}\) (\autoref{fig:linearity}; \(R^{2}=0.987\), \(n=20\), \(p<10^{-14}\)), with a  proportionality constant \(m = 0.006 \frac{\mathrm{rlu}\,\mathrm{ml}}{\mathrm{CFU}}\).
...
We measured CFU and light intensity over time for 20 drugs (\autoref{tab:drugs}) using an automated liquid handler (Methods) and estimated the distributions of the rates of change of CFU ($\psi_{\mathrm{CFU}}$) and light intensity ($\psi_I$) by bootstrap (\autoref{fig:rate_plot}, \autoref{tab:comp_stats}).
...
All time-series data are presented in Figures~\ref{fig:ampicillin_comp_combined}--\ref{fig:pex_robo}.
...
Specifically, we alternated between measuring light intensity and optical density, and exposing cultures to UV (\autoref{sec:experiments}).

UV treatment impaired bacterial growth significantly, yet the OD-normalized light intensity ($\propto$ cell-specific luminosity) of UV-treated cells was lower than that of untreated controls (\autoref{fig:SOS}d; t-test, $p = 3 \cdot 10^{-5}$ for the last time point).
...
We explored this for a subset of the antibiotics tested by imaging bacteria before and after two hours of antibiotic treatment (\autoref{fig:micro_ctrl0}--\ref{fig:micro_tri}; see Methods).
From these images, we measured bacterial length and width (\autoref{fig:filamentation_summary}) and calculated the cell volumes (\autoref{fig:volume_combined}, \autoref{sec:experiments}).

For antibiotics where microscopy showed no significant filamentation (amoxicillin, colistin, fosfomycin, rifampicin, and tetracycline, \autoref{tab:morph_post}), luminescence-based and CFU-based rates did not differ significantly (\autoref{fig:rate_plot}, \autoref{tab:comp_stats}).
...
\begin{minipage}{10cm}
\hfill
\begin{overpic}[width=.92\linewidth]{figures/volume_summary_pre.pdf}
\put(-3,18){\small\bfseries a)}%
\end{overpic}
\vspace{0cm}
\begin{overpic}[width=\linewidth]{figures/volume_summary_post.pdf}
...
Within resistance categories, bars not sharing a letter are significantly different (pairwise Tukey post hoc test, $p<0.05$; ANOVA tables and all p-values can be found in \autoref{tab:20210417U_anova} -- \autoref{tab:20220412double_tukey}).
\vspace{2.5cm}}
\label{fig:end_anova}
\end{SCfigure}
...
To further investigate the link between filamentation and the recorded light signal, we developed a simplified population dynamical model which incorporates bacterial filamentation (\autoref{sec:model}).
In this model, cells elongate linearly \cite{Wang2010}, causing the population to converge to an equilibrium cell volume, provided the division rate remains above zero (\autoref{fig:filamentation_model}).
...
As the mean cell volume stabilizes and cell-specific luminosity reaches equilibrium, the luminescence-based rate converges to the rate of change of population size $\psi_B$ (\autoref{fig:filamentation_model}).
Depending on the division and death rates, this dynamic can result in an initial peak in light intensity followed by a decline (\autoref{fig:filamentation_model}b).

This initial peak in light intensity, as predicted by the model, was experimentally observed for several drugs associated with filamentation (Figures~\ref{fig:cefepime_comp}, \ref{fig:cefuroxime_comp}, \ref{fig:doripenem_comp}, \ref{fig:imipenem_comp}, \ref{fig:mecillinam_comp}, and \ref{fig:meropenem_comp}).

To investigate the dependence of \(\psi_B\) and \(\psi_I\) on treatment-induced changes in division rate and death rate (\(\delta\)), we simulated four hours of treatment (details and parameters in \autoref{sec:model}).
...
This is evident in \autoref{fig:filamentation_model}b, where the slopes inferred from luminescence and from population size differ initially but are nearly identical at later times.
...
This approach only works if the volume-specific luminosity is constant (\autoref{sec:math}, \autoref{eq:psi_J}).
...
where \(v(t)\) is derived from the filamentation model (\eqref{eq:v_sol}, \autoref{sec:model}).
The free parameters were determined by minimizing \autoref{eq:L_mini}.
All adjusted light signals \(J\) are shown in Figures~\ref{fig:ampicillin_comp_combined}a, \ref{fig:ceftazidime_comp}, \ref{fig:ciprofloxacin_comp}, and \ref{fig:meropenem_comp}.

For ampicillin, the CFU-based and volume-corrected luminescence-based rates ($\psi_J$) did not differ significantly (\autoref{tab:comp_stats}).
...
We observed in all experiments that volume correction narrowed but never reversed the discrepancy (\(\psi_{\mathrm{CFU}} \le \psi_J \le \psi_I\), \autoref{eq:I_V_B}).
From this observation and the derivation in the SI (\autoref{sec:math}), we conclude that \(\psi_I\) is closer to \(\psi_V\) (rate of total cell volume change) than to \(\psi_B\) (rate of bacterial number change).
...
\includegraphics[width = 9cm]{figures/sensitivity_analysis.pdf}
\caption{Simulations based on the filamentation model quantifying how changes in division rate due to treatment ($\Delta \lambda$) and death rate ($\delta$) influence the rate of change of population size ($\psi_B$, blue), the rate of change of light intensity ($\psi_I$, orange), and the rate of change of light intensity when the first 2 hours of data are excluded ($\psi_{I}^*$, green).
These illustrative simulations were conducted using an initial division rate $\lambda_0 = 1.5\,\mathrm{h}^{-1}$.
More details and all parameter values can be found in \autoref{sec:model}.}
...
By definition, the rate of change of the CFU signal, $\psi_{\mathrm{CFU}}$, only matches $\psi_B$ if the number of colonies emerging per plated bacterium, $\eta \in [0, 1]$  (\autoref{eq:form_prob}), is constant over time (\autoref{sec:math}, \autoref{eq:psi_cfu}).
...
\item[c) Aggregation:] Filamentation or altered cell adhesiveness can change the size distribution of colony-initiating clusters on agar after plating (\autoref{eq:cluster_size}).
...
During ciprofloxacin treatment, CFU counts fell steeply while light intensity continued to rise (\autoref{fig:ciprofloxacin_comp}).
...
Microscopic imaging (\autoref{fig:micro_cip}) revealed almost no red fluorescence, indicating that the cells remained impermeable.
...
Trimethoprim treatment showed similar, though less pronounced, results (\autoref{fig:trimethoprim_comp}).
Trimethoprim, which impairs DNA replication (\cite{Gleckman1981}), likewise caused an increase in light intensity and a decline in CFU counts, while microscopy revealed intact, mostly impermeable, filamented cells (\autoref{fig:micro_tri}).
...
Our results show that supplementing the dilution medium increased the measured CFU substantially (\autoref{fig:PBS_suppl}, \autoref{sec:experiments}).
...
Supplementing \SI{100}{\milli M} \ce{MgCl2} yielded the highest CFU count for the first time point (\autoref{tab:pbs_ols}).
...
Given this insight into the residual killing effect of pexiganan and how to mitigate it, we repeated the CFU time-kill experiment using two different dilution media (\autoref{sec:experiments}).
...
We observed a much steeper initial decline in CFU for the cultures diluted in pure PBS compared to the supplemented ones (\autoref{fig:pexi_hand}).
In pure PBS, more highly diluted samples consistently yielded higher CFU estimates (\autoref{fig:pexi_hand}a), supporting the antimicrobial carryover hypothesis.
...
We observed no significant difference between the CFU- and luminescence-based rates for either of the tested pexiganan concentrations (\autoref{fig:rate_plot}b, \autoref{tab:comp_stats}).
However, examining the time series (\autoref{fig:pex_robo}) revealed that while CFU and luminescence signals declined in parallel for the \SI{8}{\micro\gram\per\milli\liter} treatment, they diverged for the \SI{16}{\micro\gram\per\milli\liter} kill curve.
...
During the pexiganan experiments, we observed an initial steep decline in bacterial density, predicted by both CFU and luminescence, followed by almost constant signals (Figures \ref{fig:pex_robo}, \ref{fig:pexi_hand}).
...
To investigate the first explanation, we exposed bacteria to pexiganan for \SI{5}{\minute} at \SI{16}{\micro\gram\per\milli\liter}, after which the supernatant was collected and tested for its ability to kill bacteria (\autoref{sec:experiments}).
While the initial treatment showed rapid bacterial killing ($\psi_{\text{CFU}} \approx -46\,\mathrm{h}^{-1}$ between $t_0 = 0$ and $t_1 = \SI{5}{\minute}$), bacteria exposed to the supernatant alone showed no significant reduction in viability (\autoref{fig:supernatant}, \autoref{tab:supernatant}).
